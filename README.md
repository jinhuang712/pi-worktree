...
The TUI shows linkage as a one-line widget and footer status: children show `🌲 <branch> → <origin>`, origins show only their own plus claimable unowned links (`🌲 1 worktree · wt-mine`). Other sessions' links stay out of the widget — full list lives in `/worktree status`. Result cards stay two lines; full output is one expand away.
